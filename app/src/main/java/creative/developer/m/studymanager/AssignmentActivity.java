...
public class AssignmentActivity extends Activity{
    ...
    AssignmentsList retreivedAssignemnts; // the retrieved assignments stored in the phone
    ...
    LinearLayout assignmentsLayout; // Layout that's holds all added assignments
    HashMap<Integer, LinearLayout> oneAssignmentLayouts; // layout of a single assignment
            ...
            public void onClick(View v) {
                ...
                if (!isEditing) {
                    ...
                    for (List<AssignmentsEntity> assignmentList :
                            retreivedAssignemnts.getSortedAssignments().values()) {
                        for (AssignmentsEntity assignment : assignmentList) {
                            ...
                            buttonsLayouts.get(assignment.getAssignmentID()).
                                    getChildAt(0).setVisibility(View.GONE);
                            buttonsLayouts.get(assignment.getAssignmentID()).addView(changeBtn, 1);
                            buttonsLayouts.get(assignment.getAssignmentID()).addView(deleteBtn, 2);
                        }
                    }
    ...
    protected void onActivityResult (int requestCode, int resultCode, Intent data) {
        if (requestCode == ADDING_CODE && resultCode == RESULT_OK) {
            ...
            // adding the assignment to the assignment list in the view.
            retreivedAssignemnts.addAssignment(addedAssignment);
    ...
    private void createAssignemntsView (AssignmentsList assignments) {
        ...
        for (List<AssignmentsEntity> dayList: assignments.getSortedAssignments().values()) {
    ...
    private String getDueDateStrView(int year, int month, int day) {
        ...
        String viewStr;
        System.out.println("Testing <>>" + date.get(Calendar.DAY_OF_YEAR) + " " + dueDate.get(Calendar.DAY_OF_YEAR));
        if (date.equals(dueDate)) {
            viewStr = "Due today";
        } else if (dueDate.get(Calendar.DAY_OF_YEAR) - date.get(Calendar.DAY_OF_YEAR) == 1) {
            viewStr = " Due tomorrow";
